	...
	<view class="warp">
		<view class="top_address">
			<block v-if="tabIndex == 1">
				<input class="input_css" v-model="address.name" placeholder="请输入联系人" border="none"></input>
				<input class="input_css" v-model="address.mobile" placeholder="请输入手机号" border="none"></input>
			</block>
			<block v-else>
				<view class="select_address" @tap="selectAddress">
					<view class="address-box" v-if="address===null">
						添加收货地址
					</view>
					<view class="name-tel" v-if="address!==null">
						<view class="">收货人：{{address.name}}</view>
						<view class=""> {{address.mobile}}</view>
					</view>
					<view class="address-box" v-if="address!==null">
						收货地址：{{address.areaName}}{{address.address}}
					</view>
				</view>
		...
		<view class="cen_goods">
			...
			<view class="goods" v-for="(item,index) in goods" :key="index">
				<image class="img_css" :src="item.products.images"></image>
				<view class="goods_rig">
					<view class="rig_name">{{item.products.name}}</view>
					<view class="rig_type">{{item.products.spesDesc}}</view>
					<view class="rig_bot">
						<view class="bot_num">x {{item.nums}}</view>
						<view class="bot_price">￥{{item.products.amount}}</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			getGoods(ids) {
				...
				https(urlList.getCartList, 'post', param, '获取商品').then(data => {
					this.goods = [];
					data.data.list.forEach((item, index) => {
						item.isSelect === true && this.goods.push(item)
					})
					...
					this.costFreight = data.data.costFreight;
					this.getUserDefaultShip();
				})
			},
			getUserDefaultShip() {
				https(urlList.getUserDefaultShip, 'post', {}, '获取收货地址').then(data => {
			...
			onUpOrder() {
				...
				https(urlList.createOrder, 'post', param, '提交订单').then(data => {
					uni.showToast({
						title: "提交成功,拉起支付"
					})
					uni.showModal({
